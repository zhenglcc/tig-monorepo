...
mod dead_code {   use anyhow::Ok;
   ...
   const DISTANCE_THRESHOLD: f32 = 5.66;
   const SQUARED_DISTANCE_THRESHOLD: f32 = DISTANCE_THRESHOLD * DISTANCE_THRESHOLD;
   ...
   fn filter_relevant_vectors<'a>(
       ...
       let mut heap: BinaryHeap<(FloatOrd, usize)> = BinaryHeap::with_capacity(k + 50);
   ...
   pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
       ...
       let mut distance_indices: Vec<_> = distances.iter()
           .enumerate()
           .filter(|(_, &dist)| dist > SQUARED_DISTANCE_THRESHOLD * 0.8)
           .collect();
       ...
       for &query_index in &high_distance_indices {
           ...
           for &(_, vec, index) in &r_vectors {
               ...
               if dist <= SQUARED_DISTANCE_THRESHOLD {
                   best = (dist, index);
                   break;
               }
               if dist < best.0 {
