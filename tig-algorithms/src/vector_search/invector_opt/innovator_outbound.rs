...
mod dead_code {
   ...
   fn build_kd_tree<'a>(rng: &mut SmallRng, points: &mut [(&'a [f32], usize)]) -> Option<Box<KDNode<'a>>> {
       if points.is_empty() {
           return None;
       }

       let capacity = (points.len() as f32).log2().ceil() as usize * 2;
       let mut stack = Vec::with_capacity(capacity);
       let mut root: Option<Box<KDNode<'a>>> = None;

       let compare = |axis: usize| {
           move |a: &(&[f32], usize), b: &(&[f32], usize)| {
               a.0[axis].partial_cmp(&b.0[axis]).unwrap()
           }
       };

       stack.push((0, points.len(), 0, None::<*mut KDNode>, false));
       ...
       while let Some((start, end, depth, parent_ptr, is_left)) = stack.pop() {
           ...
           let axis = rng.gen_range(0..points[0].0.len());

           let median = (start + end) >> 1;
           let comp = compare(axis);
           quickselect_by(&mut points[start..end], median - start, &comp);

           let (median_point, median_index) = points[median];
           let mut new_node = Box::new(KDNode {
               point: median_point,
               left: None,
               right: None,
               index: median_index,
           ...
           let new_node_ptr = &mut *new_node as *mut KDNode;
   ...
   pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
       ...
       let kd_tree = build_kd_tree(&mut rng, &mut kd_tree_vectors);
