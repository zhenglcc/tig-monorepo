...
mod dead_code {
   ...
   fn compute_variance_by_dimension(points: &[(&[f32], usize)]) -> Vec<usize> {
       let num_dimensions = points[0].0.len();
       let mut variances = vec![(0usize, 0f32); num_dimensions];
       ...
       for i in 0..num_dimensions {
           ...
           for &(point, _) in points {
               mean += point[i];
               sq_sum += point[i] * point[i];
           }
           mean /= len;
           let variance = sq_sum / len - mean * mean;
           variances[i] = (i, variance);
       }
   ...
   fn build_kd_tree<'a>(points: &mut [(&'a [f32], usize)]) -> Option<Box<KDNode<'a>>> {
       ...
       let dimension_order = compute_variance_by_dimension(points);
       ...
       while let Some((start, end, depth, parent_ptr, is_left)) = stack.pop() {
           ...
           let axis = dimension_order[depth % NUM_DIMENSIONS];
   ...
   fn early_stopping_distance(a: &[f32], b: &[f32], current_min: f32) -> f32 {
       ...
       while i + (CHUNK_SIZE - 1) < len {
           let mut chunk_sum = 0.0;
           for offset in 0..CHUNK_SIZE {
               unsafe {
                   let diff = *a.get_unchecked(i + offset) - *b.get_unchecked(i + offset);
                   chunk_sum += diff * diff;
               }
           }
           sum += chunk_sum;
   ...
   pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
       ...
       let vector_cost = if challenge.difficulty.num_queries < 240 {
           1680.0
       } else if challenge.difficulty.num_queries < 450 {
           1700.0
       } else if challenge.difficulty.num_queries > 1250 {
           1740.0
       } else {
           1720.0
       };
       let alpha = vector_cost * challenge.difficulty.num_queries as f64;
