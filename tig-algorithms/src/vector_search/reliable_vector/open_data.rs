...
mod dead_code {   use anyhow::Ok;
   ...
   pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
       ...
       for &query_index in &high_distance_indices {
           ...
           let mut best_dist = distances[query_index];
           let mut best_idx = best_indexes[query_index];
           ...
           best_indexes[query_index] = best_idx;
           distances[query_index] = best_dist;
       }

       if challenge.difficulty.better_than_baseline >= 500 {        
           let final_check_count = (query_count as f32 * 0.2) as usize;
           ...
           let final_check_indices: Vec<usize> = indices_with_distances.iter()
               .take(final_check_count)
               .map(|&(idx, _)| idx)
               .collect();

           for &query_idx in &final_check_indices {
               let query = &challenge.query_vectors[query_idx];

               let mut best_dist = distances[query_idx];
               let mut best_idx = best_indexes[query_idx];

               let sample_size = challenge.vector_database.len() / 100;
               let step = challenge.vector_database.len() / sample_size;

               for i in (0..challenge.vector_database.len()).step_by(step.max(1)) {
                   let dist = squared_euclidean_distance(query, &challenge.vector_database[i]);
                   if dist < best_dist {
                       best_dist = dist;
                       best_idx = i;
                   }
               ...
               best_indexes[query_idx] = best_idx;
               distances[query_idx] = best_dist;
           }
       }
