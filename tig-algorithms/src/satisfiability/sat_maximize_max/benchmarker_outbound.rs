    ...
    unsafe {
        loop {
            ...
            if !residual_.is_empty() {
                ...
                let i = if rand_val < (0.7f64 * (usize::MAX as f64)) as usize {
                    *residual_.get_unchecked(rand_val % residual_.len())
                } else {
                    let num_samples = (5.0 * (1.0 + (num_variables as f64 / 40000.0).ln())).min(10.0).max(2.0) as usize;
                ...
                let v = if let Some(abs_l) = zero_found {
                    abs_l
                } else if rand_val < ((0.52 + 0.1 * (num_variables as f64 / 100000.0).min(0.2)) 
