...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    loop {
        if !residual_.is_empty() {
            let i = if rng.gen_bool(0.9) {
                residual_[rng.gen_range(0..residual_.len())]
            } else {
                residual_
                    .iter()
                    .min_by_key(|&&clause_index| num_good_so_far[clause_index])
                    .cloned()
                    .unwrap_or(residual_[0])
            };
