...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    let base_interval = 50.0 - 25.0 * large_problem_scale;
    let min_interval = if large_problem_scale > 0.0 { 12.0 } else { 20.0 };
    let check_interval = (base_interval * (1.0 + (density / 3.0).ln().max(0.0))).max(min_interval) as usize;
