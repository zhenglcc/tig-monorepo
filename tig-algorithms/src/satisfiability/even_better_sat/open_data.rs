...
fn solve_large_instance(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    let (check_interval, max_random_prob, prob_adjustment_factor, smoothing_factor) = if is_large_problem {
...
fn solve_hard_instance(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    if (challenge.difficulty.clauses_to_variables_percent as f64) >= 425.0 && challenge.difficulty.num_variables <= 7500 {
