...
#[inline(always)]
pub fn update_state(state: &mut u64) -> &u64 {
    *state ^= *state << 13;
    *state ^= *state >> 7;
    state
}
...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    let mut state = u64::from_le_bytes(challenge.seed[..8].try_into().unwrap());
    ...
    for v in 0..num_variables {
        if p_single[v] {
            ...
            variables[v] = *update_state(&mut state) > half_u64
        }
    }
    ...
    loop {
        if !residual_.is_empty() {
            let i = residual_[*update_state(&mut state) as usize % residual_.len()];
            ...
            let v = if min_sad == 0 {
                if v_min_sad.len() == 1 {
                    ...
                    v_min_sad[*update_state(&mut state) as usize % v_min_sad.len()]
                }
            } else {
                if *update_state(&mut state) > half_u64 {
                    let l = c[*update_state(&mut state) as usize % c.len()];
                    (l.abs() - 1) as usize
                } else {
                    v_min_sad[*update_state(&mut state) as usize % v_min_sad.len()]
                }
