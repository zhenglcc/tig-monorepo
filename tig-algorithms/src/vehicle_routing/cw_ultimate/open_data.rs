...
pub fn solve_sub_instance(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
    ...
    for run in -1..NUM_PERTURBATIONS {
        ...
        if total_cost < challenge.baseline_total_distance {
...
fn optimize_with_2_opt(mut solution: SubSolution, distance_matrix: &Vec<Vec<i32>>) -> SubSolution {
...
fn compute_total_cost(solution: &SubSolution, distance_matrix: &Vec<Vec<i32>>) -> i32 {
    solution
        .routes
        .iter()
...
fn update_best_solution(
    best_solution: &mut Option<SubSolution>,
    minimum_cost: &mut i32,
    candidate_solution: (SubSolution, i32),
) -> bool {
