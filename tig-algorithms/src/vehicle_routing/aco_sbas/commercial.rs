...
type SubSolutionWithRating = (Vec<Vec<usize>>, i32);
...
pub fn solve_sub_instance(challenge: &SubInstance) -> Result<Option<SubSolution>> {
    ...
    let mut best_solution: Option<SubSolutionWithRating> =
    ...
    for i in 0..ITERATIONS {
        let mut solutions: Vec<SubSolutionWithRating> = Vec::with_capacity(num_ants);
    ...
    Ok(best_solution.map(|solution| SubSolution { routes: solution.0 }))
    //Ok(Some(SubSolution{routes: best_solution.unwrap().0}))
}

fn update_pheromone(
    pheromones: &mut Vec<Vec<f64>>,
    solutions: &mut Vec<SubSolutionWithRating>,
    best_solution: &mut Option<SubSolutionWithRating>,
    rho: f64,
) {
    let l_avg: f64 =
