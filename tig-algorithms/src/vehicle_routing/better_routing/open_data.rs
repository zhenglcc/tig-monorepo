...
pub fn solve_challenge(challenge: &Challenge) -> anyhow::Result<Option<Solution>> {
    ...
    for sub_instance in &challenge.sub_instances {
        let better_than_baseline = sub_instance.difficulty.better_than_baseline;
        
        let sub_solution = if better_than_baseline < 50 {
            simple_solver::solve_sub_instance_simple(sub_instance)?
        } else {
            complex_solver::solve_sub_instance_complex(sub_instance)?
        };
...
mod simple_solver {
    ...
    pub fn solve_sub_instance_simple(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
                ...
                for route_idx in 0..current_routes.len() {
                    ...
                    for i in 1..route.len() - 2 {
                        for j in i + 1..route.len() - 1 {
                            ...
                            if !is_route_time_feasible(&new_route, distance_matrix, service_time, ready_times, due_times) {
                                continue;
                            }
...
mod complex_solver {
    ...
    pub fn solve_sub_instance_complex(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
        let num_nodes = challenge.difficulty.num_nodes;
        let max_capacity = challenge.max_capacity;
        let demands = &challenge.demands;
        let distance_matrix = &challenge.distance_matrix;
                ...
                for route_idx in 0..best_routes.len() {
                    ...
                    for i in 1..route.len() - 1 {
                        for j in i + 1..route.len() - 1 {
                            ...
                            let new_route_distance = calculate_route_distance(&new_route, distance_matrix);
                            let old_route_distance = calculate_route_distance(&route, distance_matrix);
                        ...
                        for route2_idx in route1_idx + 1..best_routes.len() {
                            ...
                            for i in 1..route1.len() - 1 {
                                ...
                                for j in 1..route2.len() - 1 {
                                    ...
                                    let route1_demand: i32 = route1[1..route1.len()-1].iter().map(|&n| demands[n]).sum();
                                    let route2_demand: i32 = route2[1..route2.len()-1].iter().map(|&n| demands[n]).sum();
                                    
                                    let new_route1_demand = route1_demand - demands[node1] + demands[node2];
                                    let new_route2_demand = route2_demand - demands[node2] + demands[node1];
                                    
                                    if new_route1_demand > max_capacity || new_route2_demand > max_capacity {
                                        continue;
                                    }
                                    ...
                                    if !is_route_feasible(&new_route1, distance_matrix, service_time, ready_times, due_times) ||
                                      !is_route_feasible(&new_route2, distance_matrix, service_time, ready_times, due_times) {
                                        continue;
                                    }
                                    
                                    let old_distance = calculate_route_distance(&route1, distance_matrix) + 
                                                     calculate_route_distance(&route2, distance_matrix);
                                    let new_distance = calculate_route_distance(&new_route1, distance_matrix) + 
            ...
            while merge_improved && merge_iterations < max_merge_iterations {
                ...
                for i in 0..best_routes.len() {
                    ...
                    for j in 0..best_routes.len() {
                        ...
                        if route1.len() <= 2 || route2.len() <= 2 { continue; }
