...
pub fn solve_sub_instance(challenge: &SubInstance) -> Result<Option<SubSolution>> {
    ...
    for (s, i, j) in scores {
        ...
        new_distance += d[*new_route.last().unwrap()][0];

        if new_distance > challenge.baseline_total_distance {
            continue;
        }

        let (start, end) = (*new_route.first().unwrap(), *new_route.last().unwrap());
        routes[start] = Some(new_route.clone());
        routes[end] = Some(new_route.clone());
...
// Old gpu code which is no longer compatible
#[cfg(none)]
mod dead_code {
    use super::*;
    use cudarc::driver::*;
    ...
    pub fn cuda_solve_challenge(
        ...
        for k in 0..(n * (n - 1)) / 2 {
            ...
            if let (Ok(i), Ok(j)) = (
                usize::try_from(scores_flat[k * 3 + 1]),
                usize::try_from(scores_flat[k * 3 + 2]),
            ) {
                scores.push((score, i, j));
            }
        }
