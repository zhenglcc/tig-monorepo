...
pub fn solve_sub_instance(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
    ...
    for init_value in [1.0, 2.0] {
        ...
        for _ in 0..NUM_ITERATIONS {
            let neighbor_params = generate_neighbor(&current_params, &mut rng, stagnation_factor);
...
fn recompute_and_sort_savings(
    ...
    for (score, i, j) in savings_list.iter_mut() {
        let i = *i as usize;
        let j = *j as usize;
        *score = (params[i] + params[j]) * (distance_matrix[0][i] as f32
            + distance_matrix[j][0] as f32
            - distance_matrix[i][j] as f32);
    }

    // Sort by descending order of the score.
    savings_list.sort_unstable_by(|a, b| b.0.partial_cmp(&a.0).unwrap_or(std::cmp::Ordering::Equal));
}
...
#[inline]
fn two_opt_all_routes(solution: &mut SubSolution, distance_matrix: &Vec<Vec<i32>>) -> bool {
...
unsafe fn two_opt_unsafe(route: &mut Vec<usize>, distance_matrix: &Vec<Vec<i32>>) -> bool {
    ...
    for i in 1..(n - 2) {
        ...
        for j in (i + 1)..(n - 1) {
            ...
            let gain = distance_matrix[r_im1][r_i] 
                      + distance_matrix[r_j][r_jp1]
...
unsafe fn try_inter_route_swap_unsafe(
    ...
    for i in 0..num_routes {
        for j in (i + 1)..num_routes {
            ...
            if let Some((improvement, new_route_i, new_route_j)) = unsafe_find_best_swap_with_value(
                route_i,
                route_j,
                distance_matrix,
                demands,
