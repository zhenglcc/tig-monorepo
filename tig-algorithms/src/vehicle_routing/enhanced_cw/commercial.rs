...
pub fn solve_sub_instance(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
    ...
    for _ in 0..2 {
        ...
        for i in 0..NUM_ITERATIONS {
            let neighbor_params = generate_neighbor(&current_params, &mut rng, i, NUM_ITERATIONS);
...
#[inline]
fn create_initial_savings_list(challenge: &SubInstance, raw_savings: &mut [Vec<f32>]) -> Vec<(u32, u8, u8)> {
    let num_nodes = challenge.difficulty.num_nodes;
...
fn recompute_and_sort_savings(
    ...
    unsafe {
        for (score, i, j) in savings_list.iter_mut() {
            let i = *i as usize;
            let j = *j as usize;
            *score = !((*params.get_unchecked(i) + *params.get_unchecked(j)) * raw_savings.get_unchecked(i).get_unchecked(j)).to_bits();
        }
...
fn create_solution(
    ...
    for &(_, i, j) in savings_list {
        ...
        let (Some(left_route), Some(right_route)) = (routes[i].as_ref(), routes[j].as_ref()) else {
            continue;
        };
