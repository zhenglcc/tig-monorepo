...
fn generate_neighbor<R: Rng + ?Sized>(
    ...
    let base_prob = 0.5 * (-4.0 * progress).exp() + 0.04;
    let max_steps = 2;

    let mut result = current.to_vec();
    let mut modified_indices = Vec::with_capacity(challenge.difficulty.num_nodes / 2);
    ...
    let mut pairs = Vec::with_capacity(3 * challenge.difficulty.num_nodes);
