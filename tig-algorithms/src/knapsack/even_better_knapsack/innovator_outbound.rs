...
fn try_ejection(
    ...
    for &(idx, value, weight, ratio) in ejection_candidates {
        ...
        for (other_idx, &other_item) in selected_items.iter().enumerate() {
            if other_idx != idx {
                item_interactions += challenge.interaction_values[item][other_item];
            }
        }
        
        let target_interaction = challenge.interaction_values[item][target];
    ...
    for &item in &items_after_ejection {
        interaction_gain += challenge.interaction_values[target][item];
    }
    ...
    if target_value + interaction_gain > value_lost || 
       ...
       rng.gen::<f32>() < 0.98) {
        ...
        for &idx in &items_to_eject {
            ...
            *total_weight -= challenge.weights[ejected_item];
            
            unsafe {
                for x in 0..challenge.difficulty.num_items {
                    *contribution_list.get_unchecked_mut(x) -= 
                        *challenge.interaction_values.get_unchecked(ejected_item).get_unchecked(x);
                }
            }
        }
        ...
        *total_weight += challenge.weights[target];
        
        unsafe {
            for x in 0..challenge.difficulty.num_items {
                *contribution_list.get_unchecked_mut(x) += 
                    *challenge.interaction_values.get_unchecked(target).get_unchecked(x);
            }
        }
        ...
        *total_value = 0;
        ...
        for i in 0..selected_items.len() {
            for j in (i+1)..selected_items.len() {
                ...
                *total_value += challenge.interaction_values[item1][item2];
            }
        }
...
fn try_swap_pair(
    ...
    for i in 0..selected_indices.len() {
        for j in (i+1)..selected_indices.len() {
            ...
            let combined_weight = challenge.weights[item1] + challenge.weights[item2];
            let combined_value = contribution_list[item1] + contribution_list[item2];
            let internal_interaction = challenge.interaction_values[item1][item2];
            ...
            for u in 0..unselected_indices.len() {
                for v in (u+1)..unselected_indices.len() {
                    ...
                    let new_combined_weight = challenge.weights[cand1] + challenge.weights[cand2];
                    
                    if *total_weight - combined_weight + new_combined_weight > challenge.max_weight {
                        continue;
                    }
                    
                    let new_combined_value = contribution_list[cand1] + contribution_list[cand2];
                    let new_internal_interaction = challenge.interaction_values[cand1][cand2];
                    ...
                    for (idx, &sel_item) in selected_items.iter().enumerate() {
                        if idx != sel_idx1 && idx != sel_idx2 {
                            interaction_diff -= challenge.interaction_values[item1][sel_item];
                            interaction_diff -= challenge.interaction_values[item2][sel_item];
                            
                            interaction_diff += challenge.interaction_values[cand1][sel_item];
                            interaction_diff += challenge.interaction_values[cand2][sel_item];
                        }
                    }
                    ...
                    if total_diff > 0 || (total_diff >= -5 && rng.gen::<f32>() < 0.1) {
                        ...
                        *total_weight = *total_weight - combined_weight + new_combined_weight;
                        ...
                        unsafe {
                            for x in 0..challenge.difficulty.num_items {
                                *contribution_list.get_unchecked_mut(x) -= 
                                    *challenge.interaction_values.get_unchecked(sel_item1).get_unchecked(x);
                                *contribution_list.get_unchecked_mut(x) -= 
                                    *challenge.interaction_values.get_unchecked(sel_item2).get_unchecked(x);
                                *contribution_list.get_unchecked_mut(x) += 
                                    *challenge.interaction_values.get_unchecked(unsel_item1).get_unchecked(x);
                                *contribution_list.get_unchecked_mut(x) += 
                                    *challenge.interaction_values.get_unchecked(unsel_item2).get_unchecked(x);
                            }
                        }
    ...
    for iter_count in 0..local_search_iterations {
            ...
            if !feasible_adds.is_empty() {
                let pick = if rng.gen::<f32>() < 0.1 + (iter_count as f32 / local_search_iterations as f32) * 0.5 {
                    ...
                    for &add_idx in &feasible_adds {
                        ...
                        let gain = contribution_list[item];
                        if gain > best_gain {
                            best_gain = gain;
                            best_idx = add_idx;
                        }
        ...
        if !improved || rng.gen::<f32>() < 0.6 {
            ...
            for (j, &rem_item) in selected_items.iter().enumerate() {
                ...
                let rem_val = contribution_list[rem_item];
                ...
                for (i, &cand_item) in unselected_items.iter().enumerate() {
                    ...
                    let cand_val = contribution_list[cand_item];
                    
                    if cand_val < rem_val * 98 / 100 && cand_w >= rem_w {
                        continue;
                    }

                    let val_diff = cand_val - rem_val - challenge.interaction_values[cand_item][rem_item];
            ...
            if !feasible_swaps.is_empty() {
                ...
                total_weight = total_weight + new_item_weight - remove_item_weight;
                improved = true;
                no_improve_count = 0;
        ...
        if !improved {
            ...
            if no_improve_count >= max_no_improve || iter_count % std::cmp::max(15 - no_improve_count * 2, 5) == 0 {
                let mut high_value_items = Vec::with_capacity(challenge.difficulty.num_items / 2);
                
                for &item in unselected_items.iter() {
                    let item_weight = challenge.weights[item];
                    
                    if total_weight + item_weight <= challenge.max_weight {
                        continue;
                    }
                    
                    let item_value = contribution_list[item];
                ...
                for candidate_idx in 0..candidates_to_try {
                    ...
                    for (idx, &item) in selected_items.iter().enumerate() {
                        let item_weight = challenge.weights[item];
                        let effective_value = contribution_list[item];
                    ...
                    for max_items_to_remove in 1..=6.min(ejection_candidates.len()) {
                        ...
                        if max_items_to_remove > 1 {
                            ...
                            if max_items_to_remove > 2 {
                                ...
                                if try_ejection(&mut selected_items, unselected_items, &mut total_weight,
                                              &mut total_value, &mut best_solution, &mut best_solution_value,
                                              &mixed, target, target_value, 
                                              target_weight, needed_space,
...
pub fn solve_challenge(challenge: &Challenge) -> Result<Option<Solution>> {
    ...
    let min_ratio = 1.0 + difficulty_level as f64 / 10000.0;
        ...
        match solve_sub_instance(sub_instance, 1)? {
            Some((_, best_value)) => {
                ...
                let upper_ratio = best_value as f64 * 
                    (1.0 + threshold_multiplier * lookup_threshold(challenge.difficulty.num_items) as f64) / 
...
fn solve_sub_instance(challenge: &SubInstance, num_iterations: i32) -> Result<Option<(SubSolution, i32)>> {
    ...
    for _ in 0..num_iterations {
        ...
        let mut contribution_list = Vec::with_capacity(challenge.difficulty.num_items);
        ...
        if challenge.baseline_value <= best_value as u32 * 95 / 100 {
            break;
        }
    }

    let threshold = lookup_threshold(challenge.difficulty.num_items);
    if best_value > 0 && (challenge.baseline_value as f32) * (1.0 - threshold * 0.01) >= best_value as f32 {
        return Ok(None);
    }
