...
pub fn solve_sub_instance(challenge: &SubInstance) -> anyhow::Result<Option<SubSolution>> {
    ...
    // Calculate value-to-weight ratios and sort items based on this ratio
    ...
    // DP Array with simplified bitset for faster updates
    let mut dp = vec![0; max_weight + 1];
    let mut included = vec![0u64; num_items * ((max_weight + 1) / 64 + 1)];
    ...
    for (i, &(item_index, _)) in sorted_items.iter().enumerate() {
        ...
        for w in (item_weight..=max_weight).rev() {
            ...
            if new_value > dp[w] {
                dp[w] = new_value;
                let index = i * ((max_weight + 1) / 64 + 1) + w / 64;
                included[index] |= 1u64 << (w % 64);
            }
        }

        // Early exit if we find a valid solution that meets baseline_value
        if dp[max_weight] >= baseline_value {
            let mut selected_items = Vec::with_capacity(num_items);
            let mut w = max_weight;
            for j in (0..=i).rev() {
                let index = j * ((max_weight + 1) / 64 + 1) + w / 64;
                if (included[index] & (1u64 << (w % 64))) != 0 {
