...
fn compute_solution(
    ...
    let adaptive_exponent = base_exponent + (density_variance - 0.6).clamp(-adaptive_range, adaptive_range);
    ...
    while !item_densities.is_empty() {
        ...
        let selected_item;
        if selected_rank < list_size && !selected_items.is_empty() && top_ranks[selected_rank] < item_densities.len() {
        ...
        if !improved {
            ...
            'outer: for (j, &rem_item) in selected_items.iter().enumerate() {
                ...
                for (i, &cand_item) in unselected_items.iter().enumerate() {
                    ...
                    let val_diff = contribution_list[cand_item] - contribution_list[rem_item] - challenge.interaction_values[cand_item][rem_item];
                    if val_diff > 0 {
            ...
            if !feasible_swaps.is_empty() {
                ...
                let insert_pos = unselected_items.binary_search_by_key(&challenge.weights[remove_item], |&idx| challenge.weights[idx]).unwrap_or_else(|e| e);
                unselected_items.insert(insert_pos, remove_item);

                total_value += contribution_list[new_item] - contribution_list[remove_item] - challenge.interaction_values[new_item][remove_item];
...
pub fn solve_challenge(challenge: &Challenge) -> Result<Option<Solution>> {
    ...
    for (index, sub_instance) in challenge.sub_instances.iter().enumerate() {
        match solve_sub_instance(sub_instance, 11)? {
            Some((sub_solution, _best_value)) => {
...
fn solve_sub_instance(challenge: &SubInstance, num_iterations: i32) -> Result<Option<(SubSolution, i32)>> {
    ...
    let threshold = challenge.difficulty.better_than_baseline as f64 / 10000.0;
    let required_ratio_squared = 16.0 * (threshold + 1.0).powi(2) - 15.0;
    let guaranteed_pass_value = if required_ratio_squared > 0.0 {
        let required_ratio = required_ratio_squared.sqrt();
        (challenge.baseline_value as f64 * required_ratio) as i32
    } else {
        i32::MAX
    };

    for _outer_iter in 0..num_iterations {
        let mut best_local_solution: Option<SubSolution> = None;
        ...
        for _ in 0..k {
            ...
            if value > best_local_value {
                ...
                best_local_solution = Some(SubSolution {
        ...
        if let Some(local_solution) = best_local_solution {
            if best_local_value > best_value {
                ...
                if best_value >= guaranteed_pass_value {
                    break;
                }
            }
        }
